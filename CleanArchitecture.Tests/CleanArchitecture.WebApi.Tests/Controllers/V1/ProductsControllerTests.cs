...
public class ProductsControllerTests
{
    ...
    public async Task GetAll_ReturnsOkResult_WithApiResult()
    {
        ...
        var apiResult = Assert.IsType<ApiResult<IEnumerable<ProductDto>>>(okResult.Value);
    ...
    {
        var id = Guid.NewGuid();
        _serviceMock.Setup(s => s.GetByIdAsync(id)).ReturnsAsync(new ProductDto { Id = id });

        var result = await _controller.GetById(id);

        var okResult = Assert.IsType<OkObjectResult>(result);
        var apiResult = Assert.IsType<ApiResult<ProductDto>>(okResult.Value);
        Assert.True(apiResult.Success);
    }
    ...
    {
        _serviceMock.Setup(s => s.GetPagedAsync(1, 10)).ReturnsAsync(new PagedResult<ProductDto>(new List<ProductDto>(), 0, 1, 10));

        var result = await _controller.GetPaged(1, 10);

        var okResult = Assert.IsType<OkObjectResult>(result);
        var apiResult = Assert.IsType<ApiResult<PagedResult<ProductDto>>>(okResult.Value);
        Assert.True(apiResult.Success);
    }
}